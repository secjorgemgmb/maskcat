...
class MaskcatSPXCrossover(operators.Crossover[MaskcatSolution, MaskcatSolution]):
    ...
    def execute(self, parents: List[MaskcatSolution]) -> List[MaskcatSolution]:
        ...
        if rand <= self.probability:
            ...
            crossover_point = random.randint(0, len_of_masks)
...
class MaskcatUniformMutation(operators.Mutation[MaskcatSolution]):
    ...
    def execute(self, solution: MaskcatSolution) -> MaskcatSolution:
        ...
        preMut = solution.variables

        chromosome = random.randint(0, len(solution.variables)-1)

        rand = random.random()

        if rand <= self.probability:
            tmp = (random.randint(0, 4))
            while tmp == solution.variables[chromosome]:
                tmp = (random.randint(0, 4))
            solution.variables[chromosome] = tmp

            fd.write("Original: {}  Posicion: {}    Cambio: {}  Mutacion: {}\n".format(preMut, chromosome, tmp, solution.variables))
        fd.close()
